...
export default function CareerSelector({ options }: optionsTypes) {
    const [selected, setSelected] = useState(options[0])
    const { setCareerStack, careerStack } = useHowToStart()

    useEffect(()=> {
        setCareerStack(selected.name)
    }, [selected, careerStack])
