...
export default function IndustrySelector({ options }: optionsTypes) {
    const [selected, setSelected] = useState(options[0])
    const { setIndustry } = useGenerator()
    ...
    return (
        ...
        <Listbox value={selected} onChange={setSelected}>
            ...
            {/* Options */}
            <Listbox.Options className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-md h-80 overflow-y-scroll">
                {options.map((option) => (
                <Listbox.Option
                    key={option.id}
                    value={option}
